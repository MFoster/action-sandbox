...
export const transformJobToPhase = (jobs: Record<string, GitHubJob>) => {
  ...
  const jobMap = Object.fromEntries(
    Object.entries(jobs).map(([jobId, job]) => [
      jobId,
      job.needs ? (Array.isArray(job.needs) ? job.needs : [job.needs]) : [],
    ])
  );
  ...
  while (Object.keys(jobMap).length) {
    ...
    Object.keys(jobMap).forEach(
      (job) =>
        (jobMap[job] = jobMap[job].filter((dep) => jobMap[dep as string]))
    );
...
export const transformPhaseToNode = (
  phases: ReturnType<typeof transformJobToPhase>,
...
export function transformJobsToEdges(jobs: Record<string, GitHubJob>) {
